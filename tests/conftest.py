...
def server(
    event_loop, unused_tcp_port, application, script_name, url_scheme, inbuf_overflow, max_request_body_size,
    unix_socket, socket, routes, static, on_finish
):
    # Create a server.
    server, app = configure_server(
        application,
        host="127.0.0.1",
...
@pytest.yield_fixture
def response(
    event_loop, server, unused_tcp_port, request_method, request_path, request_data, request_headers, request_connector
):
    with aiohttp.ClientSession(connector=request_connector, loop=event_loop) as session:
        ...
        response = event_loop.run_until_complete(
            session.request(request_method, uri, data=request_data, headers=request_headers),
        )
        try:
            yield response
        finally:
